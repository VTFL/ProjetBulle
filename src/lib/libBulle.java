...
public class libBulle {
    ...
    public static ArrayList<Bulle> getBullesFromFile443(String filename){
        File file = new File(filename);
        try {
            BufferedReader bf = new BufferedReader(new FileReader(file));
            ArrayList<Bulle> ar = new ArrayList<Bulle>();
            for (String ligne = bf.readLine(); ligne != null; ligne = bf.readLine()){
                double x = Double.valueOf(ligne.split(" ")[0]);
                double y = Double.valueOf(ligne.split(" ")[1]);
                double truc1 = Double.valueOf(ligne.split(" ")[2]);
                double truc2 = Double.valueOf(ligne.split(" ")[3]);
                double truc3 = Double.valueOf(ligne.split(" ")[4]);
                ar.add(new Bulle(x,y,truc1,truc2,truc3));
            }
            return ar;
        } catch (Exception e1) {
    ...
    public static ArrayList<ArrayList<Bulle>> getBulles443FromFiles(String f1, String f2, String f3){
        ...
        ret.add(getBullesFromFile443(f1));
        ret.add(getBullesFromFile443(f2));
        ret.add(getBullesFromFile443(f3));
        return ret;
    }
}
